...
class LoginScreen(MDScreen):
    ...
    def menu_action(self):
        ...
        Snackbar(text="close and exit", font_size='25sp').show()
...
##                    Snackbar(text='welcome '+ lect_id_input, font_size='20sp').show()
        self.manager.current = 'Home'#'CourseSelet'
##                    break
##                else:
##                    if i == len(result)-1:  #program done scanning
##                        Snackbar(text='Username or password incorrect', font_size='20sp').show()
##                    self.manager.current = 'Login'
##
##        except:
##            #when noconnection established
##            Snackbar(text="problems connecting to server, check internet connection...",font_size='15sp').show()
    ...
    #start here!!!
        ...
        for course in this_courses:
            ...
            for s in st:
                ...
                sql3 = "SELECT ST_NAME,ST_SURNAME FROM student_tbl WHERE ST_ID=%s"
                data3 = (s[0],)
                cursor.execute(sql3,data3)
                details = cursor.fetchall()
                print(details) #display student name and surname
                f.write(str(s[0])+','+details[0][1]+','+details[0][0]+'\n')
        f.close()
        #close db connection
        conn.close()
...
class IndividualRepScreen(MDScreen):
    ...
    #Then come here!!!
        ...
        toast("individual report accessed saved")
